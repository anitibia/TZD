...
function pointToText(text) {
    ...
    finalText = finalText.replaceAll('\n','ПС'); // Добавляем замену для переноса строки
    return finalText;
}
...
function textToPoint(text) {
    ...
    finalText = finalText.replaceAll('впз','?');
    finalText = finalText.replaceAll('всз','!');
    finalText = finalText.replaceAll('пс','\n'); // Добавляем замену для переноса строки
    return finalText;
}
...
function AESEncrypt(phrase, key) {
    if (typeof phrase !== 'string') {
        phrase = new TextDecoder().decode(phrase);
    }
    // Фраза это текст
    // Ключ это хекс
...
function AESPrepairPhrase(phrase, key, cryptType) {
    if (cryptType) {
        ...
        const blockSize = 16;
        const padLength = blockSize - (phrase.length % blockSize);
        phrase = phrase + String.fromCharCode(padLength).repeat(padLength);
        let newPhrase = ""
        newPhrase = AESEncrypt(phrase, key);
        return newPhrase;
    } else {
        phrase = AESDecrypt(phrase, key);
        const padLength = phrase.charCodeAt(phrase.length - 1);
        // Проверяем, что padding корректен
        if (padLength > 0 && padLength <= 16) {
            return phrase.slice(0, -padLength);
        }
        phrase = textToPoint(phrase);
        return phrase;
    }
}
...
function decryptText() {
    ...
    if (key.length != 32) {
        ...
        while (decryptedText[decryptedText.length - 1] == "ф") {
